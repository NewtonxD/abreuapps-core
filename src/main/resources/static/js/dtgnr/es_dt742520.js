...
function createEventSource() {
  if (eventSource_dtgnr === null || eventSource_dtgnr === undefined) {
    ...
    eventSource_dtgnr = new EventSource(`${SERVER_IP}/see/dtgnr?clientId=${clientId}`,{withCredentials:true});
...
function closeEventSource(callServer=true){
    if(eventSource_dtgnr!==null && eventSource_dtgnr!==undefined){
        ...
        if(callServer) $.get(`${SERVER_IP}/see/dtgnr/close?clientId=${clientId}`);
    }
}
